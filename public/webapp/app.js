const API = location.origin + "/api/products";
...
function addToCart(product, variant, qty){
  ...
  if(idx>=0){ cart[idx].qty += qty; } else {
    cart.push({
      ...
      priceCashCents: variant.priceCashCents,
      priceCryptoCents: variant.priceCryptoCents,
      qty
    });
  }
...
function render(products, settings){
  ...
  products.forEach(p=>{
    ...
    row2.appendChild(qty); row2.appendChild(add);
...
async function main(){
  try{
    ...
    alert("Erreur de chargement du catalogue");
  }
  updateCartBadge();
}
